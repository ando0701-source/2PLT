...
- `DOC_SET: <doc_set_id>` MAY appear as an optional label and is treated as payload only. BOOT selection uses the trigger-selected BOOT LOADER JSON; `DOC_SET` is not used for document discovery.
